...
/*
THIS VERSION IS SLOW AS IT IS READING THE IMAGE AND SETTING ITS PIXELS
...
void readPNG(string filename,uint32_t &height,uint32_t &width,sf::Image *img){
    ...
    for(int x = 0;x < width;x++){
        for(int y = 0;y < height;y++){
        	auto pixels = image.get_pixel(x,y);
            img->setPixel(x,y,sf::Color(pixels.red,pixels.green,pixels.blue)); // Get the RGB values of x,y in the image and load them to the image
        }
    }
}
